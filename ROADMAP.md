...
## 🧬 Phase 6: AI Spec-Driven Domain Protocol (Strategic Pivot)

**Timeline**: Q3-Q4 2026+
**Status**: Goal definition — ADR-0068 (proposed) — `adr-ledger/adr/proposed/ADR-0068.md`

> **The idea**: Spectre's event contract today lives only as Rust enums (`EventType`, `AiPayload`) —
> readable by Rust services only. This phase adds a **contract/schema layer** and a **versioned,
> approvable spec registry** on top of the existing NATS bus and `spectre-ai-reactor` reasoning
> engine — it does not replace either. The result: Spectre becomes the canonical, machine- and
> human-readable protocol through which AI agents and developers/services interoperate across the
> whole fleet (neoland, ml-ops-api, sentinel, cerebro-rag, phantom-ml, owasaka), operationalizing the
> Spec-Driven Development principle from `specs/000-VISION-AND-INSIGHTS.md` §4 in code instead of
> convention.

This replaces the previous vague feature list (auto-scaling, blue-green, multi-tenancy) with a
concrete, dependency-ordered sequence. "LLM-assisted reactor decisions" and "self-healing inference
cluster" survive as **6f**, now sequenced as consumers of the contract layer rather than standalone
vision items.

### #57: 6a — Schema/Contract Layer
**Priority**: High
**Goal**: Make `EventType` (`spectre-events`) and `AiPayload` (`spectre-ai-reactor`) machine-readable outside Rust.
**Tasks**:
- [ ] Add `schemars` dependency; derive `JsonSchema` on `EventType`, `Event`, `EventMetadata`, `AiEvent`, `AiPayload`
- [ ] Schema-export step: one JSON Schema file per subject
- [ ] Generate an AsyncAPI 3.0 document (channels = NATS subjects, request/reply via `is_request()`/`response_type()`)
- [ ] ADR: AsyncAPI chosen over OpenAPI/Protobuf-gRPC — see ADR-0068 rationale
- [ ] CI gate: build fails if exported schema diverges from the live enum (mirrors ADR-0057's manifest-drift lesson)

### #58: 6b — Spec Registry (`spectre-spec-registry`)
**Priority**: High
**Goal**: Durable, versioned, approvable storage for contracts — operationalizes SDD's "humans approve Specs before agents act."
**Tasks**:
- [ ] New crate `spectre-spec-registry`
- [ ] TimescaleDB table `spec_registry` (`subject, version, schema_json, status, approved_by, adr_ref, ...`)
- [ ] Fix gap: `ai_decisions` table (claimed done in Phase 5 #54) is absent from `scripts/init-timescaledb.sql` — create it for real
- [ ] New event pair `spec.proposed.v1` / `spec.approved.v1` in `EventType`, following the existing request/response pattern
- [ ] Constraint: no row reaches `status=approved` without `adr_ref` populated

### #59: 6c — MCP Exposure (`spectre-mcp`)
**Priority**: High
**Goal**: External AI agents (Claude Code sessions, other repos) discover and invoke Spectre's domain via MCP, without reading Rust source.
**Tasks**:
- [ ] New crate `spectre-mcp` (kept separate from `spectre-proxy`'s TLS/JWT/rate-limit/Cloudflare scope)
- [ ] Tools: `spectre_list_subjects`, `spectre_get_schema(subject)`, `spectre_propose_spec`, `spectre_publish_event` (gated on approved spec), `spectre_list_recent_decisions`
- [ ] Resources: AsyncAPI doc (6a) + per-subject schemas
- [ ] NixOS module `nix/services/mcp/spectre-mcp.nix`, modeled on `nix/services/ai-reactor/default.nix`
- [ ] `.mcp.json` entry generated by a `generate-manifest` command — not hand-written (ADR-0057 lesson)

### #60: 6d — Interoperability SDKs
**Priority**: Medium
**Goal**: Non-Rust services (e.g. owasaka/Go) get typed clients without hand-rolling subject strings.
**Tasks**:
- [ ] Generation runs in the *consumer* repo's pipeline, from the AsyncAPI artifact (6a) — Spectre's responsibility ends at publishing a valid, versioned artifact
- [ ] Demonstrate round-trip for one real subject (e.g. `network.dns.threat.v1` against owasaka's Go consumer)

### #61: 6e — Cross-Service Rollout
**Priority**: Medium
**Goal**: neoland/ml-ops-api/sentinel/cerebro-rag become producers/consumers against the contract layer instead of hand-rolled JSON.
**Tasks**:
- [ ] Validate each service's current publish/consume against the exported schema (6a); flag drift
- [ ] New event types from any service must go through `spec.proposed.v1` → ADR reference → `spec.approved.v1` before entering `EventType`
- [ ] CI gate: fail build if a producer emits a payload that doesn't validate against its approved schema version

### #62: 6f — Contextual/Autonomous Reactor (existing vision, now sequenced)
**Priority**: Medium
**Goal**: Connect the reasoning engine to the contract layer instead of leaving it as a standalone vision item.
**Tasks**:
- [ ] Wire the existing extension point at `spectre-ai-reactor/src/reasoning.rs:11` (`// Extension point for LLM-assisted decisions`) to `spec_registry`/`ai_decisions`
- [ ] Lightweight model reads ADR/decision history from TimescaleDB, enriches reasoning layer with historical context before emitting actions (Maturity Stage 2 from vision doc §5)
- [ ] Self-healing inference cluster: Spectre detects degraded models, triggers retraining via Neoland, promotes new model after ADR approval (Maturity Stage 3)

**Boundary note**: this protocol governs the **data-plane** (message shape, schema, discovery). It is
distinct from and composes underneath the **control-plane** Agent Governance Protocol already defined
in `adr-ledger/docs/PROTOCOL.md` (Identity/Policy/Decision/Accountability via Radicle/OPA/Algorand) —
see ADR-0068 for the explicit seam.
...
### Proposed (pending ADR approval)
- **Phase 6**: AI Spec-Driven Domain Protocol — ADR-0068 proposed (#57–#62)

### Task Breakdown
- ✅ **Completed**: 37 tasks (Phase 1–5)
- 🔄 **In Progress**: #46 Multi-Region
- 📋 **Proposed**: #57–#62 Phase 6 (AI Spec-Driven Domain Protocol, pending ADR-0068 approval)
...
| Date | Change |
|------|--------|
| 2026-06-24 | Phase 6 reframed as AI Spec-Driven Domain Protocol (#57–#62) — ADR-0068 proposed in adr-ledger; replaces vague feature list with schema/contract layer, spec registry, MCP exposure, SDKs, cross-service rollout, contextual reactor |
